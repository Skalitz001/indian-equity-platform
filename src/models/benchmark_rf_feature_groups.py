...
OUTPUT_DIR = Path("artifacts/rf_feature_group_benchmark")
...
def build_training_frame(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return frame.dropna(
        subset=[*FEATURE_COLUMNS, *MARKET_CONTEXT_FEATURE_COLUMNS, "target"]
...
def benchmark_ticker(ticker: str, repo: MarketDataRepository) -> list[dict]:
    ...
    for group_name, group_columns in MARKET_CONTEXT_FEATURE_GROUPS.items():
...
def aggregate_summary(frame: pd.DataFrame) -> dict:
    ...
    for group_name, group_frame in frame.groupby("group_name"):
        summary[group_name] = {
            ...
            "classification_improved_count": int(group_frame["classification_improved"].sum()),
...
def main():
    parser = argparse.ArgumentParser(
        description="Benchmark RF feature groups against the baseline feature set.",
    )
    parser.add_argument(
        "--ticker",
        action="append",
    ...
    frame.to_csv(OUTPUT_DIR / "rf_feature_group_results.csv", index=False)
