...
OUTPUT_DIR = Path("artifacts/factor_changepoint_feature_benchmark")
...
FACTOR_FEATURE_GROUPS = {
    "factor_residual": FACTOR_CHANGEPOINT_FEATURE_COLUMNS,
    "factor_residual_context": [
        *MARKET_CONTEXT_FEATURE_COLUMNS,
        *FACTOR_CHANGEPOINT_FEATURE_COLUMNS,
    ],
    "factor_residual_context_cp": [
        *MARKET_CONTEXT_FEATURE_COLUMNS,
        *CHANGEPOINT_FEATURE_COLUMNS,
        *FACTOR_CHANGEPOINT_FEATURE_COLUMNS,
    ],
...
def prepare_universe_feature_frames(
    ...
    for ticker in tickers:
        raw = repo.load(ticker)
        frame = build_experimental_feature_frame(raw, dropna=False).copy()
...
def build_training_frame(
    ...
    frame = feature_frames[ticker].copy()
    factor_frame = build_cross_sectional_factor_frame(
        feature_frames,
        exclude_ticker=ticker,
    )
    frame = add_factor_residual_changepoint_features(frame, factor_frame)
...
def benchmark_ticker(
    ...
    for group_name, group_columns in FACTOR_FEATURE_GROUPS.items():
...
def main():
    parser = argparse.ArgumentParser(
        description="Prototype factor-residual changepoint feature groups.",
    )
    parser.add_argument(
        "--ticker",
        action="append",
    ...
    for ticker in tickers:
        rows.extend(benchmark_ticker(ticker, feature_frames))

    frame = pd.DataFrame(rows).sort_values(["group_name", "ticker"]).reset_index(drop=True)
    summary = aggregate_summary(frame)

    frame.to_csv(OUTPUT_DIR / "factor_changepoint_feature_results.csv", index=False)
